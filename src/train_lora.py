...
def load_whisper(model_name="openai/whisper-large-v3", device="cuda"):
...
def rollout_single(
    ...
    def compute_cos(origin_audio_vectors: list[torch.Tensor], gen_audio_vectors: list[torch.Tensor]) -> torch.Tensor:
        ...
        assert len(origin_audio_vectors) == len(gen_audio_vectors),
